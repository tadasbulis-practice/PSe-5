using Lab7.Implementations.Adapter;
using Lab7.Implementations.Menu;
using Lab7.Implementations.Printer;
using Lab7.Implementations.Repository;
using Lab7.Implementations.Strategy;
using Lab7.Interfaces;
using Lab7.Models;
using Lab7.Services;

namespace Lab7;

class Program
{
    static void Main(string[] args)
    {
        const string apiUrl = "http://localhost:6001";
        const bool useApi = false;
        const bool useLogging = true;

        IStudentRepository repository = useApi
            ? new ApiStudentRepository(apiUrl)
            : useLogging
                ? new LoggedStudentRepository()
                : new MemoryStudentRepository();

        IStudentPrinter printer = new ConsoleStudentPrinter();
        IAverageStrategy strategy = new SimpleAverageStrategy();
        IStudentValidator validator = new StudentValidatorAdapter();

        var service = new StudentService(repository, printer, strategy, validator);

        IRepository<Student> genericRepository = new MemoryRepository<Student>();

        var demoStudent = new Student(
            100,
            "Mohamed",
            "Faouzi",
            "mohamed@uni.lt",
            "Software Systems",
            2025);

        demoStudent.AddGrade(10);
        demoStudent.AddGrade(9);

        genericRepository.Add(demoStudent);

        var foundDemo = genericRepository.GetById(100);

        if (foundDemo != null)
        {
            Console.WriteLine($"Generic repository demo: {foundDemo.FullName}");
        }

        Student normalStudent = new Student(
            200,
            "Normal",
            "Student",
            "normal@uni.lt",
            "Software Systems",
            2025);

        Student graduateStudent = new GraduateStudent(
            201,
            "Graduate",
            "Student",
            "graduate@uni.lt",
            "Software Systems",
            2025,
            "SOLID in C#");

        Console.WriteLine(normalStudent.Info());
        Console.WriteLine(graduateStudent.Info());

        IMenuService menu = new ConsoleMenuService(service);
        menu.Run();
    }
}