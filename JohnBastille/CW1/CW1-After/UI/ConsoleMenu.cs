...
public class ConsoleMenu
{
    ...
    public ConsoleMenu(IStudentService studentService, ReportService reportService)
    {
        _studentService = studentService;
        _report_service_check(reportService);
        _reportService = reportService;
    }

    // small local check to avoid introducing Console I/O into services
    private void _report_service_check(ReportService rs)
    {
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "6":
                    ...
                    // use validator via service pattern: validate by trying to re-add or call validator directly if exposed
                    StudentValidator validator = new StudentValidator(new InMemoryStudentRepository());
                    var errors6 = validator.Validate(st6);
